<template>
  <div class="quiz-mode">
    <div class="quiz-header">
      <h1 class="mode-title">答题模式</h1>
      <div class="quiz-actions">
        <button class="action-button restart" @click="startNewQuiz">
    ...
    <div class="quiz-container">
      ...
      <div v-else-if="error" class="error-state">
        ...
        <p>{{ error }}</p>
        <button class="retry-button" @click="startNewQuiz">重试</button>
      </div>
      
      <!-- Empty State -->
      <div v-else-if="!currentQuestion" class="empty-state">
        <i class="fas fa-lightbulb"></i>
        <p>开始答题，提升你的 Spring Boot 技能！</p>
        <button class="start-button" @click="startNewQuiz">开始答题</button>
      </div>
...
export default {
  ...
  async created() {
    ...
    if (!this.currentQuestion) {
      ...
      // Initialize jump index to current question
      this.jumpToIndex = this.currentIndex + 1;
    }
  },
  beforeUnmount() {
