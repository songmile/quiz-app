<template>
  <section class="page">
    <header class="row between"><h2>统计概览</h2><button class="btn" :disabled="loading" @click="load">刷新</button></header>
...
async function load() {
  loading.value = true;
  error.value = "";
  try { data.value = await getOverview(); } catch (e) { error.value = (e as Error).message; } finally { loading.value = false; }
}
onMounted(load);
</script>
